...
public class SimpleGridObject extends UIGridXmlObject {
	...
	/**
	 * Overridato di modo tale che, in caso l'oggetto sia un'immagine, il risultato elaborato
	 * cominci con "$SMEUP" e finisca con "$".
	 */
	...
	public Object getFormattedValueForCellJXLS(int aRowIndex, String aColName)
	{
		...
		if (vColumnIndex != -1)
		{
			...
			int vDeclaredLength = vColumn.getLength() + (vColumn.getNumDec() + 1); // Qui tengo conto del segno (se numero intero getNumDec= -1,
			...
			else
			{
				...
				//			if (vValue != null && vValue.length() > vDeclaredLength && vDeclaredLength > 0)
				//			{
				...
				if (vCol != null)
				{
					...
					int vIntLun = 3;
					int vDecLun = 0;
					// E' un campo numerico se la lung è in formato NNN;NN o se il tipo è NR, in quest'ultimo caso se contiene la "," ne deduco il formato
					// Intero,Decimale
					...
					{
						if (vColType.endsWith("*YYMD") || vColType.endsWith("*YMD") || vColType.endsWith("*DMYY") || vColType.endsWith("*DMY"))
						{
					...
					else if (vColType.equalsIgnoreCase("J4IMG"))
					{
						...
						else if (vValue != null)
						{
							...
							if (vSolvedPath != null && FileUtility.fileExists(UIVariableManager.getVarValue(vSolvedPath))) {
					...
					else if (vCol.isInstantColumn())
					{
						...
						else// if("1".equalsIgnoreCase(vCol.getParametroSmeup()) || "1".equalsIgnoreCase(vCol.getParametroSmeup()))
						{
	...
	/**
	 * Override del metodo getFormattedColumnValues, di modo tale che ritorni un'ArrayList.
	 */
